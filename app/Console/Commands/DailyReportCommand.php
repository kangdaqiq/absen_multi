...
class DailyReportCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($kelasWithGroupId as $kelas) {
            ...
            // Format Class-Specific Message
            $msgClass = "📊 *Laporan Absensi Kelas {$kelas->nama_kelas}*\n";
            $msgClass .= "📅 Tanggal: " . now()->format('d/m/Y') . "\n";
            $msgClass .= str_repeat("─", 30) . "\n";
            $msgClass .= "✅ Siswa Masuk: $masuk\n";
            $msgClass .= "❌ Siswa Tidak Masuk: $tidakMasuk\n";
            $msgClass .= str_repeat("─", 30) . "\n\n";
            ...
            if ($tidakMasuk > 0) {
                ...
                foreach (['A', 'I', 'S', 'B'] as $status) {
                    if (empty($absentByStatusClass[$status])) {
                        continue;
                    }

                    $count = count($absentByStatusClass[$status]);
                    $msgClass .= "*{$statusLabels[$status]}* ({$count} siswa)\n";
                    foreach ($absentByStatusClass[$status] as $nama) {
                        $msgClass .= "  • $nama\n";
                    }
                    $msgClass .= "\n";
                }
            } else {
                $msgClass .= "🎉 *Nihil (Semua Masuk)*\n\n";
            }

            $msgClass .= "_Generated by System_";
        ...
        if ($targetJid) {
            // Format Global Message (all classes)
