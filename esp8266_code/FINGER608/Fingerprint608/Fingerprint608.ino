...
// PIN Fingerprint UART (Hardware UART pins)
#define FINGER_RX 3 // RX (GPIO3) - Hubungkan ke TX Sensor
#define FINGER_TX 1 // TX (GPIO1) - Hubungkan ke RX Sensor

#define RESPONSE_DISPLAY_TIME 3000
#define STANDBY_SCROLL_INTERVAL 500
#define BACKLIGHT_TIMEOUT 10000
#define FINGER_COOLDOWN_TIME 500
#define CARD_COOLDOWN_TIME 1000
...
// SoftwareSerial removed
Adafruit_Fingerprint finger = Adafruit_Fingerprint(&Serial);

bool sendFingerToServer(int fingerId, String scannedAt = "");
bool sendRfidToServer(String uid, String scannedAt = "");
...
bool saveOfflineFinger(int fingerId) {
...
bool saveOfflineRfid(String uid) {
  { /* Serial.println("[QUEUE] Simpan offline UID: " + uid); */ }
  unsigned long nowMs = millis();
  bool newBatch = (offlineBatchTime.length() == 0) ||
                  (lastOfflineScanMs > 0 &&
                   nowMs - lastOfflineScanMs > OFFLINE_BATCH_TIMEOUT);
...
void syncOfflineQueue() {
  ...
  while (src.available()) {
    ...
    if (fingerId == -1 && strlen(uid) == 0)
      continue;

    idx++;
    String apiUrl = String(cfg.apiUrl) + (fingerId != -1 ? "/api/fingerprint" : "/api/rfid");
    ...
    if (fingerId != -1) req["finger_id"] = fingerId;
    else req["uid"] = uid;
...
void showOfflineSavedDisplay(String uid) {
  setLcdState(LCD_OFFLINE_SAVED);
  resetActivityTimer();
  lcd.clear();
  lcd.setCursor(0, 0);
...
void handleEnrollment(int id, String type) {
  ...
  // Cek apakah jari ini sudah terdaftar di database
  p = finger.fingerFastSearch();
  if (p == FINGERPRINT_OK) {
    // Jari sudah terdaftar!
    ...
    // Kirim ID yang sudah ada ke server sebagai status sukses
    sendEnrollSuccess(existingId);
    
    // Tunggu sampai jari diangkat sebelum kembali ke standby
    ...
    sendEnrollError(id, "Error verifikasi jari");
    showStandbyDisplay();
    return;
  }
  // Jika sampai sini, berarti p == FINGERPRINT_NOTFOUND (jari belum terdaftar). Lanjutkan enroll!
  // ===========================
...
void setupWebConfig() {
  ...
  server.on("/delete-finger", HTTP_OPTIONS, []() {
...
void loop() {
  ...
  if (currentLcdState != LCD_ENROLLING) {
    uint16_t id, conf;
    int16_t fpStatus = identifyFingerprint(id, conf);
    if (fpStatus == FINGERPRINT_OK) {
      if (millis() - lastScanTime < FINGER_COOLDOWN_TIME &&
        ...
        if (WiFi.status() == WL_CONNECTED && serverOnline) {
          ...
          if (!sendFingerToServer(id, "")) {
            serverOnline = false;
            saveOfflineFinger(id);
    ...
    } else if (fpStatus == FINGERPRINT_NOTFOUND) {
      showErrorDisplay("Tak Terdaftar");
      delay(1000);
      // Tunggu sampai jari diangkat
      while (finger.getImage() != FINGERPRINT_NOFINGER) {
        feedWatchdog();
  ...
  if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
    ...
    if (uid == lastScannedUID && now - lastRFIDScanTime < CARD_COOLDOWN_TIME) {
      // Cooldown
    } else {
      lastScannedUID = uid;
...
bool sendFingerToServer(int fingerId, String scannedAt) {
  feedWatchdog();
  { /* Serial.println("[HTTP] Mengirim Finger ID: " + String(fingerId)); */ }
  String apiUrl = String(cfg.apiUrl) + "/api/fingerprint";
  WiFiClient *client = nullptr;
  if (apiUrl.startsWith("https")) {
    WiFiClientSecure *secClient = new WiFiClientSecure();
...
bool sendRfidToServer(String uid, String scannedAt) {
  feedWatchdog();
  { /* Serial.println("[HTTP] Mengirim RFID UID: " + uid); */ }
