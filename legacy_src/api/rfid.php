...
// WhatsApp API Config
define('WA_API_URL', 'http://192.168.100.67:3000/send/message'); // Sesuaikan dengan URL API Anda
define('WA_API_USER', 'admin'); // Sesuaikan dengan username Anda
define('WA_API_PASS', '04112000'); // Sesuaikan dengan password Anda
...
function sendWhatsAppNotification($phone, $message) {
    global $pdo; // Pastikan $pdo accessible
    file_put_contents('debug_rfid.log', date('Y-m-d H:i:s') . " [INFO] Sending to: $phone\n", FILE_APPEND);
    ...
    try {
        ...
        if (!$formattedPhone) {
            ...
            file_put_contents('debug_rfid.log', date('Y-m-d H:i:s') . " [WARN] Invalid Phone: $phone\n", FILE_APPEND);
            return false;
        }
        ...
        $stmt = $pdo->prepare("
            ...
            VALUES (?, ?, 'pending', NOW())
        ");
        $stmt->execute([$formattedPhone, $message]);
        
        // Return true karena kita anggap "sent to queue" sebagai sukses dari sisi user experience
        return true;
        
    } catch (PDOException $e) {
        error_log("WA Queue Error: " . $e->getMessage());
        file_put_contents('debug_rfid.log', date('Y-m-d H:i:s') . " [ERROR] " . $e->getMessage() . "\n", FILE_APPEND);
        return false;
    }
}
