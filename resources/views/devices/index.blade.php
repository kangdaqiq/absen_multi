...
<div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between" x-data>
    <h2 class="text-title-md2 font-semibold text-gray-800 dark:text-white/90">
        <i class="fas fa-microchip text-brand-500 mr-2"></i> Manajemen Device (ESP32)
    </h2>
    <button @click="$dispatch('open-modal', 'modalTambahDevice')" class="inline-flex items-center justify-center gap-2.5 rounded-md bg-brand-500 px-10 py-3 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-10 transition">
...
<div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
    ...
    <div class="p-6.5">
                ...
                <tbody>
                    @foreach ($devices as $d)
                        <tr>
                            ...
                            <td class="border-b border-gray-100 py-5 px-4 dark:border-gray-800">
                                <code class="rounded bg-gray-100 px-2 py-1 text-sm text-brand-500 dark:bg-gray-800 dark:text-brand-300 font-mono">{{ $d->api_key }}</code>
                            </td>
                            <td class="border-b border-gray-100 py-5 px-4 dark:border-gray-800">
                                @if($d->type == 'rfid')
                            ...
                            <td class="border-b border-gray-100 py-5 px-4 dark:border-gray-800">
                                <div class="flex items-center space-x-3.5" x-data>
                                    <button class="text-orange-500 hover:text-orange-700 hover:bg-orange-50 p-2 rounded-lg transition"
        ...
        <p class="mt-4 text-sm text-gray-500 dark:text-gray-400">Note: Token digunakan oleh ESP32 (header <code class="rounded bg-gray-100 px-1 py-0.5 text-brand-500 dark:bg-gray-800">X-API-Key</code>).</p>
    </div>
</div>
    ...
    <div class="p-6">
        ...
        <div class="mb-4.5">
            ...
            <div class="flex flex-col sm:flex-row gap-2">
                <input type="text" name="api_key" id="create_api_key" required class="w-full rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-brand-500 active:border-brand-500 dark:border-form-strokedark dark:bg-form-input dark:focus:border-brand-500" />
                <button type="button" onclick="generateToken('create_api_key')" class="rounded bg-gray-200 px-4 py-3 text-sm font-medium text-black hover:bg-gray-300 dark:bg-meta-4 dark:text-white dark:hover:bg-meta-3 transition">Generate</button>
            </div>
        </div>
        ...
        <div class="mb-5.5">
            ...
            <div class="flex justify-end gap-3 border-t border-stroke pt-4 dark:border-strokedark">
                <button type="button" @click="open = false" class="rounded border border-stroke px-6 py-2 font-medium text-black hover:shadow-1 dark:border-strokedark dark:text-white transition">Batal</button>
                <button type="submit" class="rounded bg-brand-500 px-6 py-2 font-medium text-white hover:bg-opacity-90 transition">Simpan</button>
            </div>
        </form>
    </div>
</x-ui.modal>
...
<x-ui.modal id="modalEditDevice">
    <div class="p-6" x-data="{ 
        ...
        <form :action="actionUrl" method="POST">
            ...
            <div class="mb-4.5">
                <label class="mb-2.5 block text-black dark:text-white">Nama Device <span class="text-meta-1">*</span></label>
                <input type="text" name="name" x-model="name" required class="w-full rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-brand-500 active:border-brand-500 dark:border-form-strokedark dark:bg-form-input dark:focus:border-brand-500" />
            </div>
            
            <div class="mb-4.5">
                <label class="mb-2.5 block text-black dark:text-white">Token (api_key) <span class="text-meta-1">*</span></label>
                <div class="flex flex-col sm:flex-row gap-2">
                    <input type="text" name="api_key" id="edit_api_key" x-model="key" required class="w-full rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-brand-500 active:border-brand-500 dark:border-form-strokedark dark:bg-form-input dark:focus:border-brand-500" />
                    <button type="button" @click="key = generateTokenString()" class="rounded bg-gray-200 px-4 py-3 text-sm font-medium text-black hover:bg-gray-300 dark:bg-meta-4 dark:text-white dark:hover:bg-meta-3 transition">Generate</button>
                </div>
            </div>
            ...
            <div class="mb-4.5">
                <label class="mb-2.5 block text-black dark:text-white">Tipe Device</label>
                <div class="relative z-20 bg-transparent dark:bg-form-input">
                    <select name="type" x-model="type" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-brand-500 active:border-brand-500 dark:border-form-strokedark dark:bg-form-input dark:focus:border-brand-500">
            ...
            <div class="mb-5.5">
                <label class="mb-2.5 block text-black dark:text-white">Status</label>
                <div class="relative z-20 bg-transparent dark:bg-form-input">
                    <select name="active" x-model="active" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-brand-500 active:border-brand-500 dark:border-form-strokedark dark:bg-form-input dark:focus:border-brand-500">
                        <option value="1">Active</option>
                        <option value="0">Inactive</option>
                    </select>
                    <span class="absolute top-1/2 right-4 z-30 -translate-y-1/2">
            ...
            <div class="flex justify-end gap-3 border-t border-stroke pt-4 dark:border-strokedark">
                <button type="button" @click="open = false" class="rounded border border-stroke px-6 py-2 font-medium text-black hover:shadow-1 dark:border-strokedark dark:text-white transition">Batal</button>
                <button type="submit" class="rounded bg-brand-500 px-6 py-2 font-medium text-white hover:bg-opacity-90 transition">Update</button>
            </div>
        </form>
    </div>
</x-ui.modal>
...
<x-ui.modal id="modalHapusDevice">
    <div class="p-6" x-data="{ id: '', name: '', actionUrl: '' }"
        @set-delete-device.window="
            id = $event.detail.id;
            name = $event.detail.name;
            actionUrl = '{{ url('devices') }}/' + id;
        ">
        <form :action="actionUrl" method="POST">
            ...
            <div class="mb-5.5 flex items-start gap-4">
                <div class="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-danger/20 text-danger">
                    <i class="fas fa-exclamation-triangle text-xl"></i>
                </div>
                <div>
                    <h4 class="text-lg font-semibold text-black dark:text-white">Konfirmasi Hapus</h4>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Yakin ingin menghapus device <strong class="text-black dark:text-white" x-text="name"></strong>? Tindakan ini tidak dapat dibatalkan.
                    </p>
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3 border-t border-stroke pt-4 dark:border-strokedark">
                <button type="button" @click="open = false" class="rounded border border-stroke px-6 py-2 font-medium text-black hover:shadow-1 dark:border-strokedark dark:text-white transition">Batal</button>
                <button type="submit" class="rounded bg-red-500 px-6 py-2 font-medium text-white hover:bg-opacity-90 transition">Ya, Hapus</button>
            </div>
        </form>
    </div>
</x-ui.modal>
