...
@section('content')
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between" x-data="{}">
        ...
        <div class="flex flex-wrap gap-2">
            <button @click="$dispatch('open-modal', 'modalImportKelas')"
                class="inline-flex items-center justify-center gap-2.5 rounded-lg border border-gray-200 bg-white px-4 py-2 text-center font-medium text-gray-700 hover:bg-gray-50 transition dark:border-gray-800 dark:bg-gray-dark dark:text-gray-300 dark:hover:bg-gray-800">
                <i class="fas fa-file-import"></i> Impor Excel
            </button>
            <button @click="$dispatch('open-modal', 'modalTambahKelas')"
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                            ...
                            <td class="border-b border-gray-100 px-4 py-4 dark:border-gray-800">
                                <div class="flex flex-col gap-1">
                                    @if($k->waliKelas)
                                        <div>
                                            <span class="text-xs font-semibold text-gray-400 dark:text-gray-500">1:</span>
                                    ...
                                    @if($k->waliKelas2)
                                        <div>
                                            <span class="text-xs font-semibold text-gray-400 dark:text-gray-500">2:</span>
                                            <a href="{{ route('guru.index') }}"
                                                class="text-brand-500 hover:underline">{{ $k->waliKelas2->nama }}</a>
                                        </div>
                                    @endif
                                    @if(!$k->waliKelas && !$k->waliKelas2)
                            ...
                            <td class="border-b border-gray-100 px-4 py-4 dark:border-gray-800 text-center">
                                @if(!$k->is_active_attendance)
                                    <span
                                        class="inline-flex rounded-full bg-gray-100 px-3 py-1 text-xs font-medium text-gray-600 dark:bg-gray-800 dark:text-gray-400">Nonaktif</span>
                                @else
                            ...
                            <td class="border-b border-gray-100 px-4 py-4 dark:border-gray-800">
                                <div class="flex items-center justify-center gap-2" x-data="{}">
                                    ...
                                    <button
                                        ...
                                        data-wali="{{ $k->wali_kelas_id }}" data-wali2="{{ $k->wali_kelas_2_id }}" data-jurusan="{{ $k->jurusan_id }}" data-wa-group-id="{{ $k->wa_group_id }}"
                                    ...
                                    <!-- Delete -->
                                    <button
                                        class="btnHapus text-error-500 hover:text-error-700 hover:bg-error-50 p-2 rounded-lg transition"
                                        data-id="{{ $k->id }}" data-nama="{{ $k->nama_kelas }}"
        ...
        <div class="p-6">
            ...
            <form action="{{ route('kelas.store') }}" method="POST">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Wali Kelas 1</label>
                        <select name="wali_kelas_id"
                            class="w-full rounded-lg border border-gray-200 bg-transparent px-4 py-2 outline-none focus:border-brand-500 dark:border-gray-800 dark:bg-gray-900 dark:text-white select2">
                    ...
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Wali Kelas 2</label>
                        <select name="wali_kelas_2_id"
                            class="w-full rounded-lg border border-gray-200 bg-transparent px-4 py-2 outline-none focus:border-brand-500 dark:border-gray-800 dark:bg-gray-900 dark:text-white select2">
                            <option value="">-- Pilih Wali Kelas 2 --</option>
                            @foreach($gurus as $g)
                                <option value="{{ $g->id }}">{{ $g->nama }}</option>
                            @endforeach
                        </select>
        ...
        <div class="p-6">
            ...
            <form action="#" method="POST" id="formEditKelas">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Wali Kelas 1</label>
                        <select name="wali_kelas_id" id="edit_wali_kelas"
                            class="w-full rounded-lg border border-gray-200 bg-transparent px-4 py-2 outline-none focus:border-brand-500 dark:border-gray-800 dark:bg-gray-900 dark:text-white select2">
                            <option value="">-- Pilih Wali Kelas 1 --</option>
                            @foreach($gurus as $g)
                                <option value="{{ $g->id }}">{{ $g->nama }}</option>
                    ...
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Wali Kelas 2</label>
                        <select name="wali_kelas_2_id" id="edit_wali_kelas_2"
                            class="w-full rounded-lg border border-gray-200 bg-transparent px-4 py-2 outline-none focus:border-brand-500 dark:border-gray-800 dark:bg-gray-900 dark:text-white select2">
                            <option value="">-- Pilih Wali Kelas 2 --</option>
                            @foreach($gurus as $g)
                                <option value="{{ $g->id }}">{{ $g->nama }}</option>
                ...
                <div class="mt-6 flex justify-end gap-3">
                    <button type="button" @click="open = false"
                        class="rounded-lg border border-gray-200 px-4 py-2 text-gray-700 hover:bg-gray-50 dark:border-gray-800 dark:text-gray-300 dark:hover:bg-gray-800">Batal</button>
                    <button type="submit"
                        class="rounded-lg bg-brand-500 px-4 py-2 text-white hover:bg-brand-600">Update</button>
                </div>
            </form>
        </div>
    </x-ui.modal>
    ...
    <x-ui.modal id="modalHapusKelas" :is-open="false">
        <div class="p-6">
            ...
            <form action="#" method="POST" id="formHapusKelas">
                @csrf
                @method('DELETE')
                <p class="text-gray-700 dark:text-gray-300 mb-6">Yakin ingin menghapus kelas: <strong id="hapus_nama_kelas"
    ...
    <x-ui.modal id="waGroupModal" :is-open="false" class="max-w-2xl">
        <div class="p-6">
            ...
            <div id="waGroupLoading" class="text-center py-8">
                <div class="inline-block h-8 w-8 animate-spin rounded-full border-4 border-solid border-brand-500 border-r-transparent align-[-0.125em]"
                    role="status">
                    <span
            ...
            <div id="waGroupError"
                class="hidden mb-4 rounded-lg bg-error-50 p-4 text-sm text-error-700 dark:bg-error-500/15 dark:text-error-500">
            </div>
            ...
            <div class="mt-6 flex justify-end">
                <button type="button" @click="open = false"
                    class="rounded-lg bg-gray-100 px-4 py-2 text-gray-700 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700">Tutup</button>
            </div>
        </div>
    </x-ui.modal>

    <!-- Modal Import -->
    <x-ui.modal id="modalImportKelas" :is-open="false">
        <div class="p-6">
            <div class="flex items-center justify-between mb-5">
                <h3 class="text-xl font-bold text-gray-800 dark:text-white/90">Impor Data Kelas</h3>
                <button @click="open = false"
                    class="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white"><i
                        class="fas fa-times"></i></button>
            </div>

            <div class="mb-6 rounded-lg bg-brand-50 p-4 text-sm text-brand-700 dark:bg-brand-500/10 dark:text-brand-400">
                <h4 class="mb-2 font-semibold"><i class="fas fa-info-circle mr-1"></i> Petunjuk Impor:</h4>
                <ul class="list-inside list-disc space-y-1">
                    <li>Gunakan template Excel yang telah disediakan.</li>
                    <li>Kolom <strong>Nama Kelas</strong> wajib diisi.</li>
                    <li>Kolom <strong>Wali Kelas</strong> bersifat opsional, pilih dari dropdown yang tersedia.</li>
                    <li>Pastikan nama guru di kolom Wali Kelas sudah terdaftar di sistem.</li>
                </ul>
            </div>
            ...
            <form action="{{ route('kelas.import') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Pilih File Excel
                            (.xlsx, .xls)</label>
                        <input type="file" name="fileExcel" accept=".xlsx, .xls, .csv" required
                            class="w-full rounded-lg border border-gray-200 bg-transparent px-2 py-1.5 outline-none focus:border-brand-500 dark:border-gray-800 dark:bg-gray-900 dark:text-white">
                    </div>

                    <div class="flex items-center gap-2 text-sm">
                        <a href="{{ route('kelas.template') }}" class="font-medium text-brand-500 hover:underline">
                ...
                <div class="mt-8 flex justify-end gap-3">
                    ...
                    <button type="submit" class="rounded-lg bg-brand-500 px-4 py-2 text-white hover:bg-brand-600">
                        <i class="fas fa-upload mr-1"></i> Mulai Impor
                    </button>
                </div>
            </form>
        </div>
