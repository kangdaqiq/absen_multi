...
@section('content')
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('rekap.exportDetail', ['id' => $siswa->id, 'start_date' => $startDate, 'end_date' => $endDate]) }}"
                class="inline-flex items-center justify-center gap-2.5 rounded-lg bg-success-500 px-4 py-2 text-center font-medium text-white hover:bg-success-600 transition">
                <i class="fas fa-file-excel"></i> Export Excel
            </a>
            <a href="{{ route('rekap.index', ['start_date' => $startDate, 'end_date' => $endDate]) }}"
    ...
    <div class="mb-6 rounded-2xl border border-gray-200 bg-white shadow-theme-sm dark:border-gray-800 dark:bg-gray-dark">
        ...
        <div class="p-5">
            <table class="w-full text-sm text-left">
                ...
                <tr>
                    <th class="py-2 text-gray-600 dark:text-gray-400 font-medium">NISN</th>
                ...
                <tbody class="text-sm">
                    @forelse($attendance as $row)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                            <td
                                class="border-b border-gray-100 px-4 py-4 dark:border-gray-800 xl:pl-6 text-gray-500 dark:text-gray-400">
                                {{ \Carbon\Carbon::parse($row->tanggal)->locale('id')->isoFormat('dddd, D MMMM Y') }}
                            </td>
                            <td
                                class="border-b border-gray-100 px-4 py-4 dark:border-gray-800 text-gray-500 dark:text-gray-400">
                                {{ $row->jam_masuk ?? '-' }}
                            </td>
                            <td
                                class="border-b border-gray-100 px-4 py-4 dark:border-gray-800 text-gray-500 dark:text-gray-400">
                            ...
                            <td class="border-b border-gray-100 px-4 py-4 dark:border-gray-800 text-center">
                                @if($row->status == 'H') <span
                                    class="inline-flex rounded-full bg-success-50 px-2.5 py-1 text-xs font-medium text-success-600 dark:bg-success-500/15 dark:text-success-500">Hadir</span>
                                @elseif($row->status == 'I') <span
                                    class="inline-flex rounded-full bg-info/10 px-2.5 py-1 text-xs font-medium text-info">Izin</span>
                                @elseif($row->status == 'S') <span
                                    class="inline-flex rounded-full bg-warning/10 px-2.5 py-1 text-xs font-medium text-warning">Sakit</span>
                                @elseif($row->status == 'T') <span
                                    class="inline-flex rounded-full bg-warning/10 px-2.5 py-1 text-xs font-medium text-warning">Terlambat</span>
                                @elseif($row->status == 'B') <span
                                    class="inline-flex rounded-full bg-danger/10 px-2.5 py-1 text-xs font-medium text-danger">Bolos</span>
                                @else <span
                                    class="inline-flex rounded-full bg-danger/10 px-2.5 py-1 text-xs font-medium text-danger">Alpha</span>
                                @endif
                            </td>
                            <td
                                class="border-b border-gray-100 px-4 py-4 dark:border-gray-800 text-gray-500 dark:text-gray-400">
