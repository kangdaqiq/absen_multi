                ...
                <x-ui.modal id="modalManageShift-{{ $s->id }}" :is-open="false" class="max-w-2xl">
                    <div class="p-6" x-data="{
                        ...
                        onSelectTeacher() {
                            ...
                            const teacher = this.allTeachers.find(t => t.id == this.selectedToAdd);
                            if (teacher && !this.assignedTeachers.some(t => t.id === teacher.id)) {
                        ...
                        <form action="{{ route('shifts.mapping.assign') }}" method="POST" class="mt-4 space-y-4">
                            ...
                            <!-- Searchable Dropdown Guru (Ketik / Cari langsung Masuk ke List) -->
                            <div class="rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50 p-3.5"
                                x-data="{
                                    searchQuery: '',
                                    isOpen: false,
                                ...
                                <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
                                    <div class="relative flex-1">
                                        ...
                                        <!-- Floating Autocomplete Dropdown List with Multi-Choice -->
                                        <div x-show="isOpen && filteredAvailable.length > 0" 
                                            x-cloak 
                                            @click.away="isOpen = false"
                                            class="absolute left-0 right-0 top-full mt-1 max-h-64 overflow-y-auto rounded-xl border border-gray-200 bg-white shadow-xl dark:border-gray-700 dark:bg-gray-900 z-50 divide-y divide-gray-100 dark:divide-gray-800">
                                            ...
                                            <div class="p-2 bg-gray-50 dark:bg-gray-800/80 flex items-center justify-between text-[11px] text-gray-500 border-b border-gray-100 dark:border-gray-700 sticky top-0 z-10">
                                                <span>Klik untuk memilih guru (bisa pilih banyak):</span>
                                            ...
                                            <template x-for="at in filteredAvailable" :key="at.id">
                                                <div @click.stop="selectTeacher(at)" 
                                                    class="flex items-center justify-between p-2.5 hover:bg-brand-50/80 dark:hover:bg-brand-500/15 cursor-pointer transition group">
                                                    <div class="flex items-center gap-2.5">
                                                        <div class="w-5 h-5 rounded border border-gray-300 group-hover:border-brand-500 flex items-center justify-center text-transparent group-hover:text-brand-500 transition text-[10px]">
                            ...
                            <div>
                                ...
                                <template x-if="assignedTeachers.length > 0">
                                    <div class="max-h-72 overflow-y-auto rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-dark shadow-sm">
                                        <table class="w-full table-auto text-left text-xs">
                                            <thead class="sticky top-0 bg-gray-50 dark:bg-gray-800/90 text-gray-600 dark:text-gray-300 font-semibold border-b border-gray-200 dark:border-gray-800">
                                                <tr>
                                ...
                                <template x-if="assignedTeachers.length === 0">
                                    <div class="p-6 text-center rounded-xl border border-dashed border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-dark">
                                        ...
                                        <p class="text-[11px] text-gray-400 mt-0.5">Pilih guru pada dropdown di atas untuk memasukkan guru ke shift ini.</p>
                                    </div>
                                </template>
                            </div>
