...
@section('content')
    <div x-data="{
        showModal: false,
        selectedPackageId: '{{ $activeSubscription?->package_id ?? ($packages->first()?->id ?? '') }}',
        isSubmitting: false,
        packages: @js($packages->map(fn($p) => ['id' => $p->id, 'name' => $p->name, 'price' => $p->price_monthly])),
        get selectedPackage() {
                ...
                <div class="rounded-2xl border border-stroke bg-white p-6 shadow-sm dark:border-strokedark dark:bg-boxdark">
                    ...
                    <ul class="space-y-3">
                        <li class="flex items-center gap-3">
                            <span
                                class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full {{ $school->wa_enabled ? 'bg-green-100 dark:bg-green-900/30' : 'bg-red-100 dark:bg-red-900/30' }}">
                                <i
                                    class="fas fa-{{ $school->wa_enabled ? 'check' : 'times' }} text-xs {{ $school->wa_enabled ? 'text-green-600' : 'text-red-500' }}"></i>
                            </span>
                            <div>
                        ...
                        <li class="flex items-center gap-3">
                            <span
                                class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full {{ $school->bot_enabled ? 'bg-green-100 dark:bg-green-900/30' : 'bg-red-100 dark:bg-red-900/30' }}">
                                <i
                                    class="fas fa-{{ $school->bot_enabled ? 'check' : 'times' }} text-xs {{ $school->bot_enabled ? 'text-green-600' : 'text-red-500' }}"></i>
                            </span>
                            <div>
                                ...
                                <p class="text-xs text-gray-400">
                                    ...
                                    @if($school->bot_enabled)
                                        —
                                        {{ $usage['bot_users']['current'] }}/{{ $usage['bot_users']['limit'] > 0 ? $usage['bot_users']['limit'] : '∞' }}
                                        guru
                                    @endif
                                </p>
                            </div>
                        ...
                        <li class="flex items-center gap-3">
                            <span
                                class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-blue-100 dark:bg-blue-900/30">
                    ...
                    <div class="p-0">
                        @if($history->isEmpty())
                            <div class="flex flex-col items-center justify-center py-16 text-center px-6">
                                ...
                                <p class="mt-1 text-sm text-gray-400">Riwayat aktivasi dan perpanjangan paket akan muncul di
                                    sini.</p>
                            </div>
                        @else
                            <div class="overflow-x-auto">
                                <table class="w-full text-sm">
                                    <thead>
                                        <tr
                                            class="border-b border-stroke dark:border-strokedark bg-gray-50 dark:bg-gray-800/50 text-left">
                                            <th
                                                class="px-6 py-3 font-semibold text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                                Paket</th>
                                            <th
                                                class="px-6 py-3 font-semibold text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                                Siklus</th>
                                        ...
                                        @foreach($history as $sub)
                                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition">
                                                ...
                                                <td class="px-6 py-4">
                                                    @if($sub->status === 'paid')
                                                        <span
                                                            class="inline-flex rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-700 dark:bg-green-900/30 dark:text-green-400">
                                                            Aktif
                                                        </span>
                                                    @elseif($sub->status === 'unpaid')
                                                        <span
                                                            class="inline-flex rounded-full bg-yellow-100 px-2.5 py-0.5 text-xs font-medium text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400">
                                                            Menunggu Pembayaran
                                                        </span>
                                                    @else
                                                        <span
                                                            class="inline-flex rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-600 dark:bg-gray-800 dark:text-gray-400">
                                                            {{ ucfirst($sub->status) }}
                                                        </span>
                                                    @endif
                                                </td>
            ...
            <div class="mt-8">
                ...
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach($packages as $pkg)
                        ...
                        <div
                            class="relative flex flex-col justify-between rounded-2xl border {{ $isActive ? 'border-brand-500 ring-2 ring-brand-500/20' : 'border-stroke dark:border-strokedark' }} bg-white p-6 shadow-sm dark:bg-boxdark transition hover:shadow-md">
                            <div>
                                @if($isActive)
                                    <span
                                        class="absolute top-4 right-4 inline-flex rounded-full bg-brand-500 px-2.5 py-0.5 text-xs font-semibold text-white">
                                        Paket Anda
                                    </span>
                                @endif
                                ...
                                <div class="mb-4">
                                    <span class="text-2xl font-extrabold text-brand-500">
                                        Rp
                                        {{ $pkg->price_monthly > 0 ? number_format($pkg->price_monthly, 0, ',', '.') : 'Gratis' }}
                                    </span>
                                ...
                                <ul class="space-y-2 text-sm text-gray-600 dark:text-gray-400 mb-6">
                                    <li class="flex items-center gap-2">
                                        ...
                                        Siswa: <strong
                                            class="text-black dark:text-white">{{ $pkg->student_limit > 0 ? number_format($pkg->student_limit) : 'Unlimited' }}</strong>
                                    </li>
                                    <li class="flex items-center gap-2">
                                        <i class="fas fa-check text-green-500 w-4"></i>
                                        Guru: <strong
                                            class="text-black dark:text-white">{{ $pkg->teacher_limit > 0 ? number_format($pkg->teacher_limit) : 'Unlimited' }}</strong>
                                    </li>
                                    <li class="flex items-center gap-2">
                                        <i
                                            class="fas fa-{{ $pkg->wa_enabled ? 'check text-green-500' : 'times text-red-400' }} w-4"></i>
                                        Notifikasi WhatsApp
                                    </li>
                                    <li class="flex items-center gap-2">
                                        <i
                                            class="fas fa-{{ $pkg->bot_enabled ? 'check text-green-500' : 'times text-red-400' }} w-4"></i>
                                        Bot WA Interaktif
                ...
                {{-- Bantuan --}}
                <div
                    class="mt-6 rounded-2xl border border-brand-500/30 bg-brand-500/5 p-5 dark:border-brand-500/20 dark:bg-brand-500/10 max-w-3xl">
                    <div class="flex gap-3">
                        <i class="fas fa-info-circle text-brand-500 mt-0.5 shrink-0"></i>
                        <div>
                            <p class="text-sm font-semibold text-brand-600 dark:text-brand-400 mb-1">Ingin upgrade paket?</p>
                            <p class="text-xs text-gray-600 dark:text-gray-400">Hubungi Developer kami untuk melakukan
                                upgrade paket.</p>
                            @if($school->operator_phone)
                                <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $school->operator_phone) }}" target="_blank"
                                    ...
                                    <i class="fab fa-whatsapp"></i> Hubungi Developer
                                </a>
                            @endif
                        </div>
                    </div>
        ...
        <div x-show="showModal" style="display: none;"
            class="fixed inset-0 z-99999 flex items-center justify-center bg-black/50 p-4 sm:p-0">
            <div x-show="showModal" @click.outside="showModal = false"
                class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:bg-boxdark"
                ...
                <div class="mb-4 flex items-center justify-between">
                    <h3 class="text-xl font-bold text-black dark:text-white">Instruksi Pembayaran</h3>
                    <button @click="showModal = false" class="text-gray-500 hover:text-black dark:hover:text-white">
                ...
                <div class="space-y-4 text-sm text-gray-600 dark:text-gray-400">
                    <div
                        class="mb-2 rounded-lg border border-stroke bg-gray-50 p-4 dark:border-strokedark dark:bg-gray-800">
                        <p class="mb-1 text-sm font-medium text-black dark:text-white">Perpanjangan Paket</p>
                        <p>Paket: <strong class="text-black dark:text-white" x-text="selectedPackage?.name"></strong></p>
                        <p>Nominal: <strong class="text-brand-500 font-bold"
                                x-text="'Rp ' + Number(selectedPackage?.price || 0).toLocaleString('id-ID') + '/bulan'"></strong>
                        </p>
                    </div>

                    <div class="rounded-lg bg-gray-50 p-4 dark:bg-gray-800">
                        <p class="mb-3 font-semibold text-black dark:text-white">Silakan transfer ke salah satu rekening
                            berikut:</p>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div
                                class="flex items-center gap-3 rounded-lg border border-stroke bg-white p-3 dark:border-strokedark dark:bg-boxdark">
                                <div
                                    class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-blue-100 text-blue-600 dark:bg-blue-900/30">
                                    <i class="fas fa-university"></i>
                                </div>
                                <div>
                                    <p class="font-bold text-black dark:text-white">
                                        {{ env('DEVELOPER_BANK_1_NAME', 'BCA') }}</p>
                                    <p class="text-sm font-mono font-bold text-brand-500">
                                        {{ env('DEVELOPER_BANK_1_ACCOUNT', '1234567890') }}</p>
                                    <p class="text-[10px] text-gray-500">a.n.
                                        {{ env('DEVELOPER_BANK_1_OWNER', 'Developer Sistem') }}</p>
                                </div>
                            </div>
                            ...
                            <div
                                class="flex items-center gap-3 rounded-lg border border-stroke bg-white p-3 dark:border-strokedark dark:bg-boxdark">
                                <div
                                    class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-orange-100 text-orange-600 dark:bg-orange-900/30">
                                    <i class="fas fa-university"></i>
                                </div>
                                <div>
                                    <p class="font-bold text-black dark:text-white">
                                        {{ env('DEVELOPER_BANK_2_NAME', 'BNI') }}</p>
                                    <p class="text-sm font-mono font-bold text-brand-500">
                                        {{ env('DEVELOPER_BANK_2_ACCOUNT', '1234567890') }}</p>
                                    <p class="text-[10px] text-gray-500">a.n.
                                        {{ env('DEVELOPER_BANK_2_OWNER', 'Developer Sistem') }}</p>
                                </div>
                            </div>
                            ...
                            <div
                                ...
                                <div>
                                    <p class="font-bold text-black dark:text-white">
                                        {{ env('DEVELOPER_BANK_3_NAME', 'Seabank') }}</p>
                                    <p class="text-sm font-mono font-bold text-brand-500">
                                        {{ env('DEVELOPER_BANK_3_ACCOUNT', '1234567890') }}</p>
                                    <p class="text-[10px] text-gray-500">a.n.
                                        {{ env('DEVELOPER_BANK_3_OWNER', 'Developer Sistem') }}</p>
                                </div>
                            </div>
                            ...
                            <div
                                ...
                                <div>
                                    <p class="font-bold text-black dark:text-white">
                                        {{ env('DEVELOPER_BANK_4_NAME', 'Dana') }}</p>
                                    <p class="text-sm font-mono font-bold text-brand-500">
                                        {{ env('DEVELOPER_BANK_4_ACCOUNT', '081234567890') }}</p>
                                    <p class="text-[10px] text-gray-500">a.n.
                                        {{ env('DEVELOPER_BANK_4_OWNER', 'Developer Sistem') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div
                        class="rounded-lg border border-yellow-200 bg-yellow-50 p-3 text-yellow-800 dark:border-yellow-900/50 dark:bg-yellow-900/20 dark:text-yellow-500">
                        <p class="text-xs">
                            <i class="fas fa-exclamation-triangle mr-1"></i>
                            Setelah melakukan transfer, silakan konfirmasi pembayaran Anda melalui WhatsApp dengan
                ...
                <div class="mt-6 flex justify-end gap-3">
                    <button @click="showModal = false" :disabled="isSubmitting"
                        class="rounded-lg border border-stroke px-4 py-2 text-sm font-medium text-black hover:bg-gray-50 dark:border-strokedark dark:text-white dark:hover:bg-gray-800 disabled:opacity-50">
                        Batal
                    </button>
                    <button @click="confirmAndRedirect()" :disabled="isSubmitting"
                        class="inline-flex items-center gap-2 rounded-lg bg-green-500 px-4 py-2 text-sm font-medium text-white hover:bg-green-600 disabled:opacity-50">
                        <i class="fab fa-whatsapp" x-show="!isSubmitting"></i>
                        <i class="fas fa-spinner fa-spin" x-show="isSubmitting" style="display: none;"></i>
                        <span x-text="isSubmitting ? 'Memproses...' : 'Konfirmasi via WA'"></span>
                    </button>
                </div>
            </div>
        </div>
