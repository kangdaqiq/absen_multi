...
<div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark max-w-3xl">
    ...
    <form action="{{ route('super-admin.schools.subscriptions.update', [$school, $subscription]) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="p-6.5 space-y-5">
            <div>
                ...
                <select name="package_id" required class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-brand-500 active:border-brand-500 disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-brand-500">
                    ...
                    @foreach($packages as $pkg)
                        <option value="{{ $pkg->id }}" {{ old('package_id', $subscription->package_id) == $pkg->id ? 'selected' : '' }}>
                            {{ $pkg->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="status" required class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-brand-500 active:border-brand-500 disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-brand-500">
                    ...
                    <option value="paid" {{ old('status', $subscription->status) == 'paid' ? 'selected' : '' }}>Paid</option>
                    <option value="failed" {{ old('status', $subscription->status) == 'failed' ? 'selected' : '' }}>Failed</option>
            ...
            <div>
                ...
                <input type="date" name="started_at" value="{{ old('started_at', $subscription->started_at ? $subscription->started_at->format('Y-m-d') : '') }}" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-brand-500 active:border-brand-500 disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-brand-500" />
                @error('started_at')
                    <span class="text-sm text-error-500 mt-1 block">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="date" name="expired_at" value="{{ old('expired_at', $subscription->expired_at ? $subscription->expired_at->format('Y-m-d') : '') }}" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-brand-500 active:border-brand-500 disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-brand-500" />
                <p class="text-sm text-gray-500 mt-1">Jika status diubah menjadi Paid, maka tanggal berakhir ini akan diupdate ke data Sekolah.</p>
                @error('expired_at')
                    <span class="text-sm text-error-500 mt-1 block">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="date" name="paid_at" value="{{ old('paid_at', $subscription->paid_at ? $subscription->paid_at->format('Y-m-d') : '') }}" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-brand-500 active:border-brand-500 disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-brand-500" />
                @error('paid_at')
                    <span class="text-sm text-error-500 mt-1 block">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" name="amount" value="{{ old('amount', $subscription->amount) }}" placeholder="Kosongkan jika mengikuti harga otomatis paket" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-brand-500 active:border-brand-500 disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-brand-500" />
                @error('amount')
                    <span class="text-sm text-error-500 mt-1 block">{{ $message }}</span>
                @enderror
            </div>
